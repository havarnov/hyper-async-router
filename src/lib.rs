...
struct Handler {
    service: Box<
        Service<
            ...
            Error = Error,
            Future = Box<Future<Item = HttpResponse<Body>, Error = Error>>,
        >,
    >,
...
impl Router {
        ...
        >,
    {
        self.router.add(
            ...
            Handler {
                service: Box::new(service),
                methods: MethodChecker::AnyOf(methods),
            },
        );
    }
        ...
        match self.router.recognize(req.path()) {
            Ok(matched) => {
                ...
                if matched_method {
                    ...
                    Box::new(matched.handler.service.call(request).map(
                        |r| Response::from(r),
                    ))
                } else {
                    Box::new(ok(
